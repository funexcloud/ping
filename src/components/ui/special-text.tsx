...
interface SpecialTextProps {
  ...
  inView?: boolean;
  once?: boolean;
  /** 부모가 재생 시점을 제어할 때 사용 (`inView`보다 우선) */
  animate?: boolean;
  onComplete?: () => void;
}
...
export function SpecialText({
  ...
  const shouldPlay = animate !== undefined ? animate : inView ? isInView : true;
  ...
  useEffect(() => {
    ...
    return () => clearStartTimeout();
  }, [shouldPlay, hasStarted, delay, text.length]);
  ...
  return (
    <span
      ref={containerRef}
      className={cn("inline-flex h-auto min-h-[1.25rem] font-ping font-medium leading-snug", className)}
    >
      {displayText}
    </span>
  );
