...
async function main() {
  const toss = loadWithMocks('ping-toss-checkout-api.js', {
  ...
  const finalize = loadWithMocks('ping-order-finalize.js', {
    './ping-firebase-admin': {
      getPingFirestoreAdmin: () => ({
        collection: () => ({
          doc: () => ({
            get: async () => ({
              exists: true,
              data: () => ({ status: 'waiting_payment', totalAmount: 220 }),
            }),
    ...
    './ping-order-payment-alignment': {
      derivePaymentDispatchAlignment: () => ({ paymentDispatchAligned: true, mismatchReason: null, refundEligible: false }),
    },
