/** PING 부고폼 — 바닐라 날짜 피커. input.value 는 항상 YYYY-MM-DD. */
...
function ensurePop() {
    ...
    const weekdaysEl = pop.querySelector('.ping-cal-weekdays');
    ...
    pop.querySelector('.ping-cal-prev').addEventListener('click', (e) => {
        e.stopPropagation();
        viewMonth -= 1;
        if (viewMonth < 0) {
            viewMonth = 11;
    ...
    pop.querySelector('.ping-cal-next').addEventListener('click', (e) => {
        e.stopPropagation();
        viewMonth += 1;
        if (viewMonth > 11) {
            viewMonth = 0;
...
function render() {
    const el = ensurePop();
    el.querySelector('.ping-cal-title').textContent = `${viewYear}년 ${viewMonth + 1}월`;

    const daysWrap = el.querySelector('.ping-cal-days');
    const cells = monthCells(viewYear, viewMonth);
    const todayYmd = toYmd(new Date());
    const selectedYmd = activeInput && activeInput.value ? activeInput.value.trim() : '';
    ...
    cells.forEach((cell) => {
        ...
        if (selectedYmd && cell.ymd === selectedYmd) btn.classList.add('ping-cal-day--selected');
...
function bindGlobal() {
    ...
    const refit = () => {
        if (pop && pop.classList.contains('ping-cal-open') && activeInput) positionPop(activeInput);
    };
...
/**
 * @param {string[]} ids — element id 목록
 */
export function attachDatePickersById(ids) {
    ids.forEach((id) => {
